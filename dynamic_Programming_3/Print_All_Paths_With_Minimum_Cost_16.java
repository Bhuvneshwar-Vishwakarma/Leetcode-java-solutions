...
public class Print_All_Paths_With_Minimum_Cost_16 {
    	  ...
    	  if(rem.i == dp.length - 1 && rem.j == dp[0].length - 1) {
    		System.out.println(rem.psf);  
    		 ...
    		 queue.add(new Pair(rem.psf + "V", rem.i + 1, rem.j)); 
    		  ...
    		  if(dp[rem.i][rem.j + 1] < dp[rem.i + 1][rem.j]) {//horizontal is smaller
    			  queue.add(new Pair(rem.psf + "H", rem.i, rem.j + 1)); 
    			  
    		  }else if(dp[rem.i][rem.j + 1] > dp[rem.i + 1][rem.j]) {// vertical is smaller
    			  queue.add(new Pair(rem.psf + "V", rem.i + 1, rem.j)); 
